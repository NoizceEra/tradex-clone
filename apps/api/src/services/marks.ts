...
/** Skew-normalization depth floor ($1M) used until the LP pool is sized. */
const DEPTH_FLOOR = 1_000_000_000_000n;
 ...
 * live skew/depth on every trade so the price moves intraday.
 */
export async function recomputeMark(
  ...
  const { markE6, premiumE6 } = syntheticMark({
    indexE6,
    skewUusdc,
    depthUusdc: depthUusdc > 0n ? depthUusdc : DEPTH_FLOOR,
