...
/** Set (rename) the caller's chat username. Validated + globally unique (race-safe via the index). */
export async function setUsername(db: Db, userId: string, rawName: string): Promise<{ username: string }> {
  ...
  const taken = await db.query(`SELECT 1 FROM users WHERE lower(display_name) = lower($1) AND id <> $2`, [name, userId]);
