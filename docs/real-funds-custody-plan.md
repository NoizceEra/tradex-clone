...
- **Proof-of-reserves:** on-chain treasury USDC ≥ |`TREASURY_USDC`| at all times. House money (fees/insurance) is real too, so it's correctly included.
...
- `deposits(id, user_id, onchain_sig TEXT UNIQUE, asset, amount_in_raw, usdc_credited_e6 BIGINT, swap_sig, status, txn_id, observed_at, credited_at)` — **`onchain_sig UNIQUE` ⇒ idempotent crediting**. `status`: `detected → swapping → swept → credited` (USDC deposits skip `swapping`).
...
| `deposits.ts` | Scan deposit addresses (Helius webhook or poll) at **`finalized` commitment**. New `onchain_sig` → insert `deposits` row → (SOL only) **Jupiter-swap → USDC**, slippage-bounded → sweep to treasury → `creditDeposit()` posting `+USER_COLLATERAL, −TREASURY_USDC` **for the actual USDC proceeds, full amount, no clamp**, idempotently. Ignore dust below `minDepositUsd` (uneconomic to sweep; anti-dusting). **Fee mechanics:** fresh deposit addresses hold no SOL — the **hot wallet is the fee payer** for sweep txs and funds ATA rent (~0.002 SOL) where needed. Sweep promptly after crediting. |
...
| **P1** | Deposit path, **USDC-only** (no Jupiter on the critical path): HD address → scanner (`finalized`) → sweep (hot-wallet fee payer) → full-credit. | devnet, allowlisted |
| **P1.5** | SOL deposits + Jupiter auto-swap (slippage-bounded, credit actual output). | devnet |
